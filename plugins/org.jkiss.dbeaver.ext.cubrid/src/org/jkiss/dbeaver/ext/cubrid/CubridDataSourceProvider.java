...
import org.jkiss.dbeaver.ext.cubrid.model.meta.CubridMetaModelDescriptor;
...
public class CubridDataSourceProvider extends JDBCDataSourceProvider {

    private final Map<String, CubridMetaModelDescriptor> metaModels = new HashMap<>();
    private static final String EXTENSION_ID = "org.jkiss.dbeaver.cubrid.meta";

    public CubridDataSourceProvider()
    {
        metaModels.put(CubridConstants.META_MODEL_STANDARD, new CubridMetaModelDescriptor());
        ...
        for (IConfigurationElement ext : extElements) {
            CubridMetaModelDescriptor metaModel = new CubridMetaModelDescriptor(ext);
        ...
        for (CubridMetaModelDescriptor metaModel : new ArrayList<>(metaModels.values())) {
    ...
    public DBPDataSource openDataSource(
        ...
        throws DBException
    {
        CubridMetaModelDescriptor metaModel = null;
        Object metaModelId = container.getDriver().getDriverParameter(CubridConstants.PARAM_META_MODEL);
        if (metaModelId != null && !CubridConstants.META_MODEL_STANDARD.equals(metaModelId)) {
        ...
        CubridMetaModel metaModelInstance = metaModel.getInstance();
        return metaModelInstance.createDataSourceImpl(monitor, container);
    }

    protected CubridMetaModelDescriptor getStandardMetaModel() {
        return metaModels.get(CubridConstants.META_MODEL_STANDARD);
    }
    ...
    public DBPPropertyDescriptor[] getConnectionProperties(DBRProgressMonitor monitor, DBPDriver driver, DBPConnectionConfiguration connectionInfo) throws DBException {
        ...
        if (connectionProperties == null || connectionProperties.length == 0) {
            ...
            String driverParametersString = CommonUtils.toString(driver.getDriverParameter(CubridConstants.PARAM_DRIVER_PROPERTIES));
