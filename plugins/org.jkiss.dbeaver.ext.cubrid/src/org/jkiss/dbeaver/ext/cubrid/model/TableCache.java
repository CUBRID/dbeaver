...
public class TableCache extends JDBCStructLookupCache<CubridStructContainer, CubridTableBase, CubridTableColumn> {
        ...
        throws SQLException, DBException
    {
        ...
        Object d = dbResult.getObject(JDBCConstants.COLUMN_DEF); //CubridUtils.safeGetObject(columnObject, dbResult, JDBCConstants.COLUMN_DEF);
        ...
        String sql = "SHOW FULL COLUMNS from " + CubridUtils.safeGetString(tableObject, dbResult, JDBCConstants.TABLE_NAME);
		...
		while(rs.next()) {
			if(rs.getString("Field").equals(columnName)) {
				unique = rs.getString("Key").equals("UNI") ? true : false;
				collation_name = rs.getString("Collation");				
				break;
			}
		}
		
		String sql2 = "SELECT * FROM db_serial WHERE class_name = " + SQLUtils.quoteString(table, table.getName()) + " AND att_name = '" + columnName + "'";
		JDBCPreparedStatement dbStat2 = session.prepareStatement(sql2); 
		ResultSet rs2 = dbStat2.executeQuery();
		Integer initialValue = null;
		Integer incrementValue = null;
		while(rs2.next()) {
			initialValue = rs2.getInt("min_val");
			incrementValue = rs2.getInt("increment_val");
			autoIncrement = true;
		}
		
		String sql3 = "SELECT * FROM db_attribute WHERE class_name = " + SQLUtils.quoteString(table, table.getName()) + " AND attr_name = '" + columnName + "'";
		JDBCPreparedStatement dbStat3 = session.prepareStatement(sql3); 
		ResultSet rs3 = dbStat3.executeQuery();
		boolean shared = false;
		while(rs3.next()) {
			shared = (rs3.getString("attr_type")).equals("SHARED") ? true : false;
		}
