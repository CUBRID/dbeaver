...
public class CubridMetaModel {
        ...
        throws DBException
    {
        ...
        {   	
        	...
        	while(dbResult.next()) {
        		...
        		String owner = CubridUtils.safeGetStringTrimmed(procObject, dbResult, CubridConstants.OWNER);
    ...
    public JDBCStatement prepareProcedureFunctionLoadStatement(@NotNull JDBCSession session)
        throws SQLException
    {
       String sql= "SELECT * ,sp_name as PROCEDURE_NAME, sp_type as PROCEDURE_TYPE, \r\n"
       		+ "comment as REMARKS FROM db_stored_procedure";
       final JDBCPreparedStatement dbStat = session.prepareStatement(sql);
    ...
    public JDBCStatement prepareTriggersLoadStatement(@NotNull JDBCSession session, @Nullable CubridStructContainer forParent) throws SQLException {    	
    	String sql;
    	if(forParent.getDataSource().getContainer().getConnectionConfiguration().getUserName().toUpperCase().equals("DBA")) {
    		sql = "select t1.*, t2.* from db_trigger t1 JOIN db_trig t2 ON t1.name = t2.trigger_name";
    	}else {
    		sql = "select * from db_trig where owner_name = ''";
    	}
        final JDBCPreparedStatement dbStat = session.prepareStatement(sql);
        return dbStat;
    }
